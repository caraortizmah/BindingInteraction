...
do
  ...
  if [ -d "$(echo "$i" | cut -d':' -f1)" ]; then #if folder $i exists,  -d checks whether $jj directory exists or not
    ...
    ./script_check_ch.sh new_input.out
    ...
    $mopac16 comp_final_"$res".mop #comp_final2.mop
    $mopac16 comp-pep_final_"$res".mop #comp-pep_final2.mop
    $mopac16 pep_final_"$res".mop #pep_final2.mop
